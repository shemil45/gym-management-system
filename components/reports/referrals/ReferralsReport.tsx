...
import { REFERRALS_TABS, referralsSearchParams, type ReferralsReportQuery } from '@/lib/reports/referrals-params'
...
export default function ReferralsReport({ query, gymName, controls, children }: Props) {
    const tabLabel = REFERRALS_TABS.find((tab) => tab.id === query.tab)?.label ?? 'Referrals'
    const comparisonLabel = COMPARISONS.find((option) => option.id === query.compare)?.label
    const compared = query.tab === 'overview' && query.previous
    return (
        <div className="space-y-5">
            {gymName && (
                <PrintHeader
                    gymName={gymName}
                    title={`Referrals — ${tabLabel}`}
            ...
            <div className="print:hidden">
                ...
                <h1 className="mt-1 text-lg font-semibold text-gray-900 dark:text-white">Referrals</h1>
            </div>

            <ReportTabs
                ariaLabel="Referrals reports"
                tabs={REFERRALS_TABS.map((tab) => ({ id: tab.id, label: tab.label, href: `/admin/reports/referrals?${referralsSearchParams({ ...query, tab: tab.id }).toString()}` }))}
            />
