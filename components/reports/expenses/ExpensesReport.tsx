...
import { EXPENSES_TABS, expensesSearchParams, type ExpensesReportQuery } from '@/lib/reports/expenses-params'
...
export default function ExpensesReport({ query, gymName, controls, children }: Props) {
    const tabLabel = EXPENSES_TABS.find((tab) => tab.id === query.tab)?.label ?? 'Expenses'
    ...
    return (
        <div className="space-y-5">
            ...
            <ReportTabs
                ...
                tabs={EXPENSES_TABS.map((tab) => ({ id: tab.id, label: tab.label, href: `/admin/reports/expenses?${expensesSearchParams({ ...query, tab: tab.id }).toString()}` }))}
            />
