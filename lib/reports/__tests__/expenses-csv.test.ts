...
describe('expenses csv', () => {
    it('pnl has bucket start, income lines, categories, totals', () => {
        ...
        const csv = pnlCsv({ buckets: [bucket] })
    ...
    it('category and ledger rows', () => {
        const rent = { category: 'rent' as const, label: 'Rent', entries: 2, total: 1000, share: 50, avg: 500, previous: 500, delta: 100 }
        expect(categoryCsv({ rows: [rent], compare: 'previous' }).split('\r\n')[1]).toBe('Rent,2,1000,50,500,500,100')
        // With comparison off the two comparison columns are blank, not zero.
        expect(categoryCsv({ rows: [rent], compare: 'none' }).split('\r\n')[1]).toBe('Rent,2,1000,50,500,,')
        expect(ledgerCsv({ rows: [{ id: 'e', amount: 300, category: 'rent', description: 'Sept, rent', expense_date: '2026-09-01', created_at: '', receipt_url: 'https://x/r.pdf', adder_name: 'S1', is_demo: false }] }).split('\r\n')[1]).toBe('2026-09-01,Rent,"Sept, rent",300,S1,https://x/r.pdf')
    })
})
