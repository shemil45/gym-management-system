...
const row: ReportPaymentRow = {
    ...
    member_id: 'm1', member_name: 'Asha', member_code: 'GYM001', member_phone: '9999999999', member_joined: '2026-09-15', plan_name: 'Monthly', processor_name: 'S1',
...
describe('payments csv builders', () => {
    ...
    it('summary emits buckets with method columns', () => {
        ...
        const csv = summaryCsv({ buckets: [bucket] })
        expect(csv.split('\r\n')[1]).toBe('2026-09-15,15 Sep,1,1500,0,1500,0,0,0,500,1000,50,0')
    })
    it('plan, pending, staff', () => {
        expect(planCsv({ rows: [{ plan: 'Monthly', txns: 1, revenue: 1500, share: 100 }] }).split('\r\n')[1]).toBe('Monthly,1,1500,100')
        expect(pendingCsv({ rows: [{ ...row, payment_status: 'pending' }] }).split('\r\n')[1]).toBe('2026-09-15,Asha,GYM001,9999999999,Monthly,1500,upi,pending,"a, note"')
        expect(staffCsv({ rows: [{ staff: 'S1', txns: 1, collected: 1500, cash: 0 }] }).split('\r\n')[1]).toBe('S1,1,1500,0')
    })
})
